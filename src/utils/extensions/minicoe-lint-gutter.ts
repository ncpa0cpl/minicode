...
export const nextDiagnostic: Command = (view: EditorView) => {
  ...
  activateHover(view, next.from, 1, {
    tooltip: lintHover,
    until: (tr) =>
      tr.docChanged || tr.newSelection.main.head < next.from || tr.newSelection.main.head > next.to,
  });
  return true;
};
...
export const previousDiagnostic: Command = (view: EditorView) => {
  ...
  let from = prevFrom ?? lastFrom,
    to = prevTo ?? lastTo ?? from;
  view.dispatch({ selection: { anchor: from, head: to }, scrollIntoView: true });
  activateHover(view, from, 1, {
    tooltip: lintHover,
    until: (tr) =>
      tr.docChanged || tr.newSelection.main.head < from || tr.newSelection.main.head > to,
  });
  return true;
};
...
function batchResults<T>(
  ...
  for (let p of promises)
    p.then((value) => {
      ...
      else timeout = window.setTimeout(() => sink(collected), 200);
    }, error);
}
...
class LintPanel implements Panel {
        ...
        if (item) {
          ...
          for (let i = 0; i < keys.length; i++)
            if (keys[i]!.toUpperCase().charCodeAt(0) == event.keyCode) {
              ...
              const action = diagnostic.actions![i];
              if (found && action) action.apply(view, found.from, found.to);
            }
        }
      } else {
        return;
  ...
  update() {
    ...
    while (
      i < this.items.length &&
      !(this.items.length == 1 && this.items[0] && this.items[0].diagnostic.from < 0)
    ) {
      needsSync = true;
      this.items.pop();
    }
  ...
  moveSelection(selectedIndex: number) {
    ...
    if (!item) return;
    let selection = findDiagnostic(field.diagnostics, item.diagnostic);
    if (!selection) return;
    this.view.dispatch({
  ...
  if (lState && lState.diagnostics.size) {
    ...
    for (let iter = RangeSet.iter([lState.diagnostics]); ; iter.next()) {
      for (let i = 0; i < pending.length; i++) {
        const from = pendingStart[i];
        if ((!iter.value || iter.value.spec.diagnostics.indexOf(pending[i]) < 0) && from != null) {
          f(pending[i]!, from, lastEnd);
          pending.splice(i, 1);
